...
export default function MessageBubble({ type, text, choices, onSelect, isLast }) {
  ...
  return (
    <div className={`cb:flex ${isUser ? 'cb:justify-end' : 'cb:justify-start'} cb:mb-6`}>
      <div className={`cb:max-w-[90%] cb:p-4 cb:rounded-2xl cb:text-sm cb:shadow-sm ${
        ...
        {/* The Inline Actions (Only if bot message and has choices) */}
        {!isUser && choices && choices.length > 0 && (
          <div className="cb:mt-4 cb:flex cb:flex-col cb:gap-2">
            {choices.map((choice) => (
              <button
                key={choice.id}
                // Disabling old buttons prevents "Time Travel" errors
                disabled={!isLast} 
                onClick={() => onSelect(choice)}
                className={`cb:w-full cb:text-left cb:px-3 cb:py-2 cb:rounded-lg cb:border cb:transition-all ${
                  isLast 
                  ? 'cb:bg-white cb:border-blue-200 cb:text-blue-700 hover:cb:bg-blue-50 active:cb:scale-[0.98]' 
